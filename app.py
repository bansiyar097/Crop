...
def prediction():
    if request.method=="POST":
        ...
        print(nitro,phos,kp,temp,hum,ph,rainfall)
        with open("model.pkl","rb") as model_file:
            mlmodel = pickle.load(model_file)
        res = mlmodel.predict([[float(nitro),float(phos),float(kp),float(temp),float(hum),float(ph),float(rainfall)]])
        print(res)
        conn = sqlite3.connect("cropdata.db")
        cur = conn.cursor()
        cur.execute(f''' insert into crop values({nitro},{phos},{kp},{temp},{hum},{ph},{rainfall},'{res[0]}')''')
        conn.commit()
